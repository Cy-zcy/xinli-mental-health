...
const props = withDefaults(
  defineProps<{
    ...
    /** 导航栏中标题是否居中 */
    titleCenter?: boolean
    /** 是否启用导航栏，默认使用应用配置 `navbar.enable`，或者传递配置对象 */
    navbar?: boolean | NavbarConfig
    /** 导航栏是否启用边框 */
...
const showNavbar = computed(() => {
  if (typeof props.navbar === 'boolean') {
    return props.navbar ?? settingsStore.settings.navbar.enable
  }
  if (typeof props.navbar === 'object') {
    return true
  }
  return settingsStore.settings.navbar.enable
})

// 导航栏标题
const navbarTitle = computed(() => {
  return props.title || navbarConfig.value?.title || settingsStore.title
})
          ...
          <div ref="startSideRef" class="h-full flex-center whitespace-nowrap">
            <div class="h-full flex-center whitespace-nowrap px-2 empty:hidden">
              ...
              <FmButton
                ...
                class="p-2"
              >
                <FmIcon name="i-ic:round-arrow-back" class="text-lg" />
              </FmButton>
